...
docker run -d -t --restart=always --dns 127.0.0.1 -P --name master.test.com -h master.test.com -w /root test/$name-master:$tag&> /dev/null
...
do
	...
	docker run -d -t --restart=always --dns 127.0.0.1 -P --name slave$i.test.com -h slave$i.test.com -e JOIN_IP=$FIRST_IP test/$name-slave:$tag &> /dev/null
	((i++))
done
